...
- 2026-02-27: Stabilized Stripe checkout lifecycle integration test (`tests/int/checkout-flow.int.spec.ts`) by resetting Stripe mock state per test (`mockReset`) to prevent cross-test `mockResolvedValueOnce` leakage. Added assertion proving capacity rejection happens before Stripe calls. Verified with targeted int+unit runs (26 passing tests across checkout lifecycle, order service, and payment-intent suites).
- 2026-02-27: Added deliberate Stripe live test harness (`vitest.stripe-live.config.mts`, `tests/stripe-live/checkout-webhook.live.spec.ts`) with self-owned fixtures (event/order create+cleanup), Stripe API test-mode confirmation (`pm_card_visa`), and webhook-driven confirmation polling. Added `pnpm` scripts: `test:stripe:live` and `test:stripe:live:watch`. Not executed yet pending Stripe test account/webhook setup.
